...
| 选项 | 作用说明 |
| :--- | :--- |
| `--once` | **单次免存模式**：**既不读取**本地已有凭据与配置，**也绝不向本地保存**任何数据（登录凭据、开播设置与 OBS 密码等用完即焚），适合在公共设备或临时开播时使用 |
...
- **本地私有存储**：登录凭据（`auth.json`）与开播设置（`live-settings.json`）默认仅保存在本机，严格限制只有当前系统用户可读，具备防文件损坏保护。
- **推流码绝不落盘**：推流码直接从内存传给本地 OBS，绝不以明文或文件形式保存在硬盘上，从根源上杜绝推流码意外泄露。
- **支持随时登出与单次无痕模式**：随时执行 `bili-live-tui logout` 一键清除本机凭据；在公共设备上只需加 `--once` 参数，启动时不读取本地任何旧记录，运行期间与退出后也绝不保存任何数据到磁盘（用完即焚），实现彻底无痕开播。
