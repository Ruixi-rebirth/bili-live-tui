...
- **屏蔽词管理**：查看、添加和删除当前账号的弹幕观看屏蔽词。此设置用于观看过滤，不是全直播间的违禁词表，不会禁止其他观众发送这些词。
...
| 快捷键              | 功能说明                                                     |
| :------------------ | :----------------------------------------------------------- |
| `↑` / `↓`           | 切换管理栏目（房管/禁言/全局禁言/黑名单/屏蔽词）或选择表格行 |
...
`bili-live-tui` 的使用方式主要分为两种：**启动开播控制台**（主程序）与**快捷管理操作**（独立子命令），两者为独立平行的调用路径：
...
| 选项 | 作用说明 |
| :--- | :--- |
| `--once` | **单次会话模式**：不读取或保存登录凭据与开播设置（包括 OBS WebSocket 密码），不写本程序诊断日志。外部程序路径仍会读取，手动设置后仍会保存 |
...
| 命令 | 功能说明 |
| :--- | :--- |
| `bili-live-tui status` | 查询 B 站直播间开播状态、已播时长与标题分区，不检测其他进程的本地推流状态 |
| `bili-live-tui stop` | 向 B 站发送下播请求，无需进入交互界面。不直接控制其他进程中的 OBS 或 FFmpeg |
...
- **服务连接**：直播相关请求使用 Bilibili 接口，也会连接你设置的本地或远程 OBS、下载你填写的封面地址。
- **本地配置存储**：登录凭据（`auth.json`）与开播设置（`live-settings.json`）默认保存在本机。这些文件可能包含登录凭据和 OBS WebSocket 密码，请勿分享给他人。
- **推流码传递**：推流码从内存传给 OBS 或 FFmpeg，不作为本程序的开播设置保存。外部程序可能保存自己的配置或日志。
- **支持登出与单次会话**：执行 `bili-live-tui logout` 清除本机登录凭据。`--once` 隔离本程序的登录凭据与开播设置，并关闭诊断日志，但不删除已有文件。外部程序路径配置仍会读取和保存，OBS、FFmpeg、mpv 及操作系统自身的记录不受此选项控制，因此不应将它视为“彻底无痕”模式。
