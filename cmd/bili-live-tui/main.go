...
const localizedUsageTemplate = `用法:{{if .Runnable}}
  {{.CommandPath}}{{if or .HasAvailableFlags .HasAvailableInheritedFlags}} [选项]{{end}}{{if .HasAvailableSubCommands}} [命令]{{end}}{{else if .HasAvailableSubCommands}}
...
集开播设置、自动联动、实时弹幕与房管治理于一体。`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runTUI(cmd.Context(), !noSaveAuthFlag)
		},
	}

	rootCmd.SetUsageTemplate(localizedUsageTemplate)
	rootCmd.PersistentFlags().BoolVar(&noColorFlag, "no-color", false, "禁用界面颜色高亮")
	rootCmd.PersistentFlags().BoolVar(&noSaveAuthFlag, "no-save-auth", false, "临时免存模式：不读取也不向本地写入登录凭据")

	statusCmd := &cobra.Command{
		Use:   "status",
		Short: "查看当前直播间开播状态与推流状态",
		RunE: func(cmd *cobra.Command, args []string) error {
			return runStatus(cmd.Context(), !noSaveAuthFlag)
		},
	}
	...
	stopCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runStop(cmd.Context(), !noSaveAuthFlag)
		},
	}
...
func initAppContext(ctx context.Context, saveAuth bool) (*appContext, error) {
	...
	if !saveAuth || err != nil {
		auth, err = performLogin(ctx, saveAuth)
		if err != nil {
...
func runStatus(ctx context.Context, saveAuth bool) error {
	app, err := initAppContext(ctx, saveAuth)
	if err != nil {
		return err
	}
...
func runStop(ctx context.Context, saveAuth bool) error {
	app, err := initAppContext(ctx, saveAuth)
	if err != nil {
		return err
	}
...
func runTUI(ctx context.Context, saveAuth bool) error {
	app, err := initAppContext(ctx, saveAuth)
	if err != nil {
		return err
	}
