...
func TestRootCmdStructureAndCompletions(t *testing.T) {
	...
	if flag := cmd.Flags().Lookup("once"); flag == nil {
		t.Error("missing flag --once on rootCmd")
	}
	if flag := cmd.Flags().Lookup("no-color"); flag == nil {
		t.Error("missing flag --no-color on rootCmd")
	}
	for _, subName := range []string{"status", "stop", "logout"} {
		sub, _, err := cmd.Find([]string{subName})
		if err != nil || sub == nil {
			t.Fatalf("subcommand %s not found", subName)
		}
		if flag := sub.Flags().Lookup("once"); flag != nil {
			t.Errorf("subcommand %s should not inherit --once flag", subName)
		}
	}
