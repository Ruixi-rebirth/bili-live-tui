...
func TestValidateCoverInput(t *testing.T) {
	...
	t.Run("directory is rejected", func(t *testing.T) {
		dir := t.TempDir()
		err := validateCoverInput(dir, false)
		if err == nil || !strings.Contains(err.Error(), "直播封面路径必须是普通文件，不能是目录") {
...
func TestLiveEditPageAppliesResponsiveDensityLocally(t *testing.T) {
	...
	defer screen.Fini()
	app := tview.NewApplication()
	page := newLiveEditPage(app, api.LiveSettings{}, nil, nil, nil)
	page.body.SetRect(0, 0, 100, 20)
	screen.SetSize(100, 20)
	page.body.Draw(screen)
	if got := page.body.description.GetFieldHeight(); got != 1 {
		t.Fatalf("short-screen description height = %d, want 1", got)
	}

	page.body.SetRect(0, 0, 100, 32)
	screen.SetSize(100, 32)
	page.body.Draw(screen)
	if got := page.body.description.GetFieldHeight(); got != 3 {
		t.Fatalf("tall-screen description height = %d, want 3", got)
	}
}
