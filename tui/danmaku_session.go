...
func (s *LiveDanmakuSession) handleEvent(event api.DanmakuEvent) {
	...
	if event.Kind == api.DanmakuEventGuard {
		for i := len(s.history) - 1; i >= 0 && i >= len(s.history)-5; i-- {
			h := s.history[i]
			if duplicateGuardNotice(h, event) {
				return
			}
		}
	}
	...
	if event.Kind == api.DanmakuEventGift && (event.Message.GiftCombo > 1 || event.Message.BatchComboID != "") {
		for i := len(s.history) - 1; i >= 0 && i >= len(s.history)-10; i-- {
			...
			if giftMessagesMatch(h.Message, event.Message) {
				if event.Message.GiftCombo > 0 && event.Message.GiftCombo < h.Message.GiftCombo {
					return // 过期连击通知不能回退文字、时间或重复增加统计。
				}
				if event.Message.GiftCoinType == "" && h.Message.GiftCoinType != "" {
					event.Message.GiftCoinType = h.Message.GiftCoinType
				}
				var deltaCount int64
				if event.Message.GiftCombo > h.Message.GiftCombo {
					deltaCount = int64(event.Message.GiftCombo - h.Message.GiftCombo)
					h.Message.GiftCombo = event.Message.GiftCombo
				} else if event.Command != "COMBO_SEND" && event.Message.GiftCount > 0 {
					deltaCount = int64(event.Message.GiftCount)
				}
				var deltaCoin int64
				if event.Message.GiftTotalCoin > h.Message.GiftTotalCoin {
					deltaCoin = event.Message.GiftTotalCoin - h.Message.GiftTotalCoin
				...
				s.stats.ObserveComboGift(event, deltaCount, deltaCoin)
				s.historyRevision++
				s.notifyLocked()
				return
			}
...
func (s *LiveDanmakuSession) seedPopularityFromRoom(client *api.Client, roomID string) {
	...
	if err == nil && snapshot.OnlineKnown {
		// 请求期间到达的心跳比这个兜底查询新，不能用响应到达时间覆盖它。
		s.ObservePopularity(snapshot.Online, requestedAt)
	}
}
