...
func newRoomManagerWorkspace(deps roomManagerDependencies) *roomManagerWorkspace {
	...
	w.addKeywordButton = newActionButton("添加屏蔽词", func() {
		w.openInput("添加屏蔽词", "关键词 ", "", 0, "添加", func(value string) error {
			if strings.TrimSpace(value) == "" {
				return fmt.Errorf("屏蔽词不能为空")
			}
			return nil
		}, func(value string) {
			w.pendingLabel = "添加屏蔽词"
			w.pendingAction = func(ctx context.Context) error {
				return w.deps.Client.AddRoomShieldKeyword(ctx, w.deps.RoomID, strings.TrimSpace(value), w.deps.Sessdata, w.deps.BiliJCT)
			}
			w.runAction()
		})
	})
...
func (w *roomManagerWorkspace) updateActionBar(mode string) {
	...
	wasActionButton := previousFocus == w.backButton ||
		...
		previousFocus == w.addKeywordButton ||
	...
	if mode == "error" {
		...
		case "keyword":
			w.currentButtons = append(w.currentButtons, w.addKeywordButton)
		case "flow":
			if w.flowSearchable {
