...
func (body *responsiveLiveEditBody) Draw(screen tcell.Screen) {
	...
	availableForForm := max(bodyHeight-4, 1)
	padding, rows := responsiveLiveFormDensity(bodyHeight)
	body.description.SetSize(rows, 0)
	if preferredLiveFormHeight(body.form, padding) > availableForForm {
		padding = 0
	}
	body.form.SetItemPadding(padding)
	prefHeight := preferredLiveFormHeight(body.form, padding)
	formHeight := min(prefHeight, availableForForm)
	panelHeight := formHeight + 4

	body.panel.ResizeItem(body.form, formHeight, 0)
...
func newLiveEditPage(app *tview.Application, initial api.LiveSettings, areas []api.LiveArea, onSave func(api.LiveSettings), onCancel func()) *liveEditPage {
	...
	panel.AddItem(form, 0, 0, true)
	panel.AddItem(nil, 1, 0, false)
	panel.AddItem(buttons, 1, 0, true)
	panel.AddItem(status, 2, 0, false)
	...
	body := &responsiveLiveEditBody{Flex: flex, panel: panel, form: form, description: state.description}
