...
func TestRoomSnapshotRefreshLoopDoesNotOverlapRequests(t *testing.T) {
	...
	go func() {
		runRoomSnapshotRefreshLoop(context.Background(), done, time.Millisecond, refresh)
		close(stopped)
	}()
	...
	for range 3 {
		...
		case <-time.After(200 * time.Millisecond):
			t.Fatal("timed out waiting for serialized refresh")
		}
	}
	close(done)
	select {
	...
	defer mutex.Unlock()
	if maxActive != 1 {
		t.Fatalf("maximum concurrent refreshes = %d, want 1", maxActive)
	}
}
...
func TestFormatStreamHealth(t *testing.T) {
	...
	emptyText := formatStreamHealth(streamruntime.Health{})
	if emptyText != "" {
		t.Fatalf("empty health text should be empty, got %q", emptyText)
	}
}
...
func TestOverviewStopConfirmPageOrder(t *testing.T) {
	pages := tview.NewPages()
	mainPage := tview.NewBox()
	confirmModal := tview.NewModal()
	homePage := tview.NewBox()

	pages.AddPage("main", mainPage, true, true)
	pages.AddPage("confirm-stop", confirmModal, true, false)
	pages.AddPage("home", homePage, true, true)

	// 当在直播概览中触发下播确认时，需要 ShowPage 并 SendToFront
	pages.ShowPage("confirm-stop")
	pages.SendToFront("confirm-stop")

	// 模拟绘制，验证 confirm-stop 处于最顶层绘制
	...
	// confirm-stop 包含按钮，应处于可见并在最前
	name, _ := pages.GetFrontPage()
	if name != "confirm-stop" {
		t.Fatalf("expected front page to be confirm-stop, got %q", name)
	}
}
