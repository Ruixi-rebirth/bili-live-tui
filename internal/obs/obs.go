...
func Preflight(host, port, password string) error {
	...
	if isObsRunning(host, port) {
		...
		if err != nil {
			return fmt.Errorf("连接 OBS WebSocket 失败: %w。端口已连通，请确认 OBS WebSocket 密码或协议版本是否正确", err)
		}
...
func (r *Runtime) Start(rtmpAddr, streamKey string) (startErr error) {
	r.lifecycleMu.Lock()
	defer r.lifecycleMu.Unlock()
	if r.started || r.stopping {
		return fmt.Errorf("OBS 运行实例已启动或停止，不能重复启动")
	}
	r.started = true
	defer func() {
		if startErr != nil {
