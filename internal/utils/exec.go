...
func ResolveExecutableWithProbe(key, displayName string, probe ExecutableProbe, candidates ...string) (string, error) {
	...
	if configured := normalizeExecutablePathInput(paths[key]); configured != "" {
		allCandidates = append(allCandidates, configured)
	}
	for _, candidate := range candidates {
		if candidate = normalizeExecutablePathInput(candidate); candidate != "" {
	...
	return "", &ExecutableNotFoundError{
		...
		Suggested:   normalizeExecutablePathInput(paths[key]),
		Probe:       probe,
	}
}
...
func ConfigureExecutablePath(key, displayName, value string, probes ...ExecutableProbe) (string, error) {
	value = normalizeExecutablePathInput(value)
	if value == "" {
...
func normalizeExecutablePathInput(value string) string {
	...
	// FromSlash is intentionally platform-aware: Windows accepts pasted paths
	// with either separator, while Linux keeps backslashes as literal filename
	// characters instead of silently changing their meaning.
...
func readExecutablePathsUnlocked() (map[string]string, error) {
	...
	if err := ReadJSON(path, &paths); err != nil {
...
func writeExecutablePathsUnlocked(paths map[string]string) error {
	...
	return WriteJSONAtomically(path, paths)
}
