...
func (c *Client) UploadRoomCoverURL(ctx context.Context, roomID, sessdata, biliJCT, imageURL string) (string, error) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		if attempt > 0 {
			...
			case <-ctx.Done():
				return "", ctx.Err()
			case <-time.After(time.Duration(200*(1<<uint(attempt-1))) * time.Millisecond):
			}
		}
		...
		req.Header.Set("User-Agent", biliBrowserUserAgent)
		resp, lastErr = downloadHTTPClient.Do(req)
		if lastErr == nil {
			break
		}
	}
	if lastErr != nil {
		return "", fmt.Errorf("下载直播封面失败: %w", lastErr)
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return "", fmt.Errorf("下载直播封面失败：远程服务器返回 HTTP %d", resp.StatusCode)
	}
	lr := io.LimitReader(resp.Body, maxRoomCoverSourceBytes+1)
	data, err := io.ReadAll(lr)
	if err != nil {
		return "", fmt.Errorf("下载直播封面失败: %w", err)
	}
	if int64(len(data)) > maxRoomCoverSourceBytes {
		return "", fmt.Errorf("直播封面源文件不能超过 64 MB")
	}
