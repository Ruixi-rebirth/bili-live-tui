...
func GetTVQRCodeContext(ctx context.Context) (qrURL string, authCode string, err error) {
	...
	if result.Code != 0 {
		return "", "", fmt.Errorf("获取登录二维码失败（错误码 %d）：%s", result.Code, responseMessage(result.Message, result.Msg))
	}
...
func CheckQRStatusContext(ctx context.Context, authCode string) (QRStatus, *TVQRPollResponse, error) {
	...
	default:
		return status, &result, fmt.Errorf("检查扫码状态失败（错误码 %d）：%s", result.Code, responseMessage(result.Message, result.Msg))
	}
}
