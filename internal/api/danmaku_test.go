...
func TestSendDanmakuRejectsSoftBlockedMessage(t *testing.T) {
	testBodies := []string{
		`{"code":0,"message":"f","msg":"f","data":[]}`,
		`{"code":0,"message":"","msg":"","data":"f"}`,
		`{"code":0,"message":"0","msg":"0","data":["f"]}`,
	}
	for _, body := range testBodies {
		...
		if err == nil || !errors.Is(err, ErrDanmakuProhibited) || !IsDanmakuProhibited(err) {
			t.Fatalf("SendDanmaku(body=%q) error = %v, want ErrDanmakuProhibited", body, err)
		}
	}
}

func TestSendDanmakuDoesNotInspectSuccessfulResponseContentAsAnError(t *testing.T) {
	transport := roundTripFunc(func(*http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusOK,
			Body: io.NopCloser(strings.NewReader(
				`{"code":0,"message":"","msg":"","data":{"mode_info":{"extra":"{\"content\":\"敏感\"}"}}}`,
			)),
			Header: make(http.Header),
		}, nil
	})
	...
	client.BaseURL = "http://test.invalid"
	seedDanmakuSendIdentity(client, "sess")
	if err := client.SendDanmaku(context.Background(), "123", "sess", "csrf", "敏感"); err != nil {
		t.Fatalf("SendDanmaku() error = %v, want successful response data to be ignored", err)
	}
}
...
func TestGetDanmakuInfoErrorReportsCodeAndMsgFallback(t *testing.T) {
	client := NewClient(&http.Client{Transport: roundTripFunc(func(*http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusOK,
			Body:       io.NopCloser(strings.NewReader(`{"code":65530,"message":"","msg":"token错误"}`)),
			Header:     make(http.Header),
		}, nil
	})})
	client.BaseURL = "http://test.invalid"
	_, err := client.GetDanmakuInfoWithCookie(context.Background(), "1", "sess", "jct")
	if err == nil || !strings.Contains(err.Error(), "获取弹幕连接信息失败（错误码 65530）：token错误") {
