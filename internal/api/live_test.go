...
func TestGetRoomSnapshot(t *testing.T) {
	transport := roundTripFunc(func(r *http.Request) (*http.Response, error) {
		...
		return &http.Response{
			StatusCode: http.StatusOK,
			Body:       io.NopCloser(strings.NewReader(`{"code":0,"data":{"room_info":{"uid":99,"room_id":1,"title":"标题","description":"简介","tags":"游戏,聊天","area_id":376,"area_name":"单机游戏","parent_area_name":"主机游戏","live_status":1,"live_start_time":1700000000,"online":"42"},"watched_show":{"num":1234}}}`)),
			Header:     make(http.Header),
		}, nil
	})
	client := NewClient(&http.Client{Transport: transport})
	...
	if snapshot.AnchorID != "99" || snapshot.RoomID != "1" || snapshot.AreaID != "376" || snapshot.AreaName != "单机游戏" || snapshot.Online != 42 || !snapshot.OnlineKnown || snapshot.Watched != 1234 || !snapshot.WatchedKnown || snapshot.LiveTime.Unix() != 1700000000 {
...
func TestRoomManagementListResponses(t *testing.T) {
	transport := roundTripFunc(func(r *http.Request) (*http.Response, error) {
		...
		case "/xlive/web-ucenter/v1/banned/GetSilentUserList":
			...
			if values.Get("room_id") != "1" || values.Has("pn") || values.Get("ps") != "2" {
...
func TestGetRoomSnapshotFallsBackToLegacyEndpoint(t *testing.T) {
	transport := roundTripFunc(func(r *http.Request) (*http.Response, error) {
		...
		if r.URL.Path == "/room/v1/Room/get_info" {
			return &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader(`{"code":0,"data":{"room_id":1,"title":"备用标题","area_name":"单机游戏","live_status":1,"live_time":"2026-03-09 10:15:00","online":7}}`)), Header: make(http.Header)}, nil
		}
		return nil, fmt.Errorf("unexpected path %s", r.URL.Path)
	})
	client := NewClient(&http.Client{Transport: transport})
	...
	if snapshot.Title != "备用标题" || snapshot.AreaName != "单机游戏" || snapshot.Online != 7 || !snapshot.OnlineKnown || snapshot.LiveTime.IsZero() {
...
func TestUploadRoomCoverRejectsDirectory(t *testing.T) {
	client := NewClient(nil)
	dir := t.TempDir()
	_, err := client.UploadRoomCover(context.Background(), "1", "sess", "jct", dir)
	if err == nil || !strings.Contains(err.Error(), "直播封面路径必须是文件，不能是目录") {
		t.Fatalf("unexpected error for directory cover: %v", err)
	}
}
...
func TestAPIErrorsIncludeCodeAndMsgFallback(t *testing.T) {
	newMockClient := func(responseBody string) *Client {
		transport := roundTripFunc(func(*http.Request) (*http.Response, error) {
			return &http.Response{
				StatusCode: http.StatusOK,
		...
		c.BaseURL = "http://test.invalid"
		return c
	}

	t.Run("GetMyRoomID", func(t *testing.T) {
		c := newMockClient(`{"code":-101,"message":"","msg":"账号未登录"}`)
		_, err := c.GetMyRoomID(context.Background(), "sess")
		if err == nil || !strings.Contains(err.Error(), "获取我的房间号失败（错误码 -101）：账号未登录") {
			t.Fatalf("unexpected error: %v", err)
		}
	})

	t.Run("GetLiveAreas", func(t *testing.T) {
		c := newMockClient(`{"code":500,"message":"","msg":"分区获取异常"}`)
		_, err := c.GetLiveAreas(context.Background(), "token")
		if err == nil || !strings.Contains(err.Error(), "获取分区列表失败（错误码 500）：分区获取异常") {
			t.Fatalf("unexpected error: %v", err)
		}
	})

	t.Run("GetRoomSnapshot", func(t *testing.T) {
		c := newMockClient(`{"code":1001,"message":"","msg":"直播间不存在"}`)
		_, err := c.GetRoomSnapshot(context.Background(), "1")
		if err == nil || !strings.Contains(err.Error(), "获取直播状态失败（错误码 1001）：直播间不存在") {
			t.Fatalf("unexpected error: %v", err)
		}
	})

	t.Run("GetGuardTopList", func(t *testing.T) {
		c := newMockClient(`{"code":1002,"message":"","msg":"大航海受限"}`)
		_, err := c.GetGuardTopList(context.Background(), "1", 42, 1)
		if err == nil || !strings.Contains(err.Error(), "获取大航海列表失败（错误码 1002）：大航海受限") {
			t.Fatalf("unexpected error: %v", err)
		}
	})

	t.Run("GetOnlineGoldRank", func(t *testing.T) {
		c := newMockClient(`{"code":1003,"message":"","msg":"高能榜获取失败"}`)
		_, err := c.getOnlineGoldRank(context.Background(), "1", 42, "sess", "jct")
		if err == nil || !strings.Contains(err.Error(), "获取在线榜失败（错误码 1003）：高能榜获取失败") {
			t.Fatalf("unexpected error: %v", err)
		}
	})

	t.Run("GetRoomPlaybackURL", func(t *testing.T) {
		c := newMockClient(`{"code":1004,"message":"","msg":"房间未开播"}`)
		_, err := c.GetRoomPlaybackURL(context.Background(), "1", "sess", "jct")
		if err == nil || !strings.Contains(err.Error(), "获取直播预览失败（错误码 1004）：房间未开播") {
			t.Fatalf("unexpected error: %v", err)
		}
	})

	t.Run("AddLiveTag", func(t *testing.T) {
		c := newMockClient(`{"code":1005,"message":"","msg":"标签超出数量上限"}`)
		_, err := c.AddLiveTag(context.Background(), "1", "sess", "jct", "tag")
		if err == nil || !strings.Contains(err.Error(), "新增直播标签失败（错误码 1005）：标签超出数量上限") {
			t.Fatalf("unexpected error: %v", err)
		}
	})

	t.Run("DeleteLiveTag", func(t *testing.T) {
		c := newMockClient(`{"code":1006,"message":"","msg":"标签不存在"}`)
		err := c.DeleteLiveTag(context.Background(), "1", "sess", "jct", "tagID")
		if err == nil || !strings.Contains(err.Error(), "删除直播标签失败（错误码 1006）：标签不存在") {
			t.Fatalf("unexpected error: %v", err)
		}
	})

	t.Run("UpdateRoomNews", func(t *testing.T) {
		c := newMockClient(`{"code":1007,"message":"","msg":"公告包含敏感词"}`)
		err := c.UpdateRoomNews(context.Background(), "1", "sess", "jct", "news")
		if err == nil || !strings.Contains(err.Error(), "设置直播公告失败（错误码 1007）：公告包含敏感词") {
			t.Fatalf("unexpected error: %v", err)
		}
	})

	t.Run("UpdatePreLiveCover", func(t *testing.T) {
		c := newMockClient(`{"code":1008,"message":"","msg":"封面审核失败"}`)
		err := c.UpdatePreLiveCover(context.Background(), "1", "sess", "jct", "https://i.example/cover.jpg")
		if err == nil || !strings.Contains(err.Error(), "更新直播封面失败（错误码 1008）：封面审核失败") {
			t.Fatalf("unexpected error: %v", err)
		}
	})

	t.Run("UpdateLiveInfoWithCookie", func(t *testing.T) {
		c := newMockClient(`{"code":1009,"message":"","msg":"标题不合规"}`)
		err := c.UpdateLiveInfoWithCookie(context.Background(), "1", "token", "sess", "jct", LiveSettings{
			Title:  "测试",
			AreaID: "376",
		})
		if err == nil || !strings.Contains(err.Error(), "设置开播信息失败（错误码 1009）：标题不合规") {
			t.Fatalf("unexpected error: %v", err)
		}
	})
}
