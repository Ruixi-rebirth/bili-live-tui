...
func TestRemoveAuth(t *testing.T) {
	...
	// Initially no auth file exists
	removed, err := RemoveAuth()
	if err != nil {
		t.Fatalf("RemoveAuth error when file does not exist: %v", err)
	}
	if removed {
		t.Fatal("expected removed to be false when no file exists")
	}
	...
	if !removed {
		t.Fatal("expected removed to be true when file was deleted")
	}

	// Verify file is gone
	if _, err := LoadAuth(); err == nil {
		t.Fatal("LoadAuth succeeded after RemoveAuth")
	}
}
...
func TestRemoveLiveSettings(t *testing.T) {
	configHome := t.TempDir()
	t.Setenv("XDG_CONFIG_HOME", configHome)

	// 文件不存在时，应返回 false, nil
	removed, err := RemoveLiveSettings()
	if err != nil {
		t.Fatalf("RemoveLiveSettings error when file does not exist: %v", err)
	}
	if removed {
		t.Fatal("expected removed to be false when no file exists")
	}

	// 保存配置后删除，应返回 true, nil
	want := api.LiveSettings{Title: "测试直播", AreaID: "376"}
	if err := SaveLiveSettings(want); err != nil {
		t.Fatalf("SaveLiveSettings failed: %v", err)
	}

	removed, err = RemoveLiveSettings()
	if err != nil {
		t.Fatalf("RemoveLiveSettings failed: %v", err)
	}
	if !removed {
		t.Fatal("expected true when file exists and was removed")
	}

	// 验证文件确实已不存在
	path := filepath.Join(configHome, AppName, LiveSettingsFileName)
	if _, err := os.Stat(path); !os.IsNotExist(err) {
		t.Fatalf("expected file to be deleted, but stat returned: %v", err)
	}
}
