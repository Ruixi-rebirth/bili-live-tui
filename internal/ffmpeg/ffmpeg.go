...
func (r *TestRuntime) Start(rtmpAddr, streamKey string) error {
	...
	if r.started || r.stopping {
		r.mu.Unlock()
		return fmt.Errorf("FFmpeg 运行实例已启动或停止，不能重复启动")
	}
	r.started = true
	r.mu.Unlock()
...
func ffmpegStreamArgs(orientation, rtmpAddr, streamKey string) []string {
...
func newFFmpegProcess(rtmpAddr, streamKey, orientation string) (*exec.Cmd, io.ReadCloser, error) {
	...
	args := ffmpegStreamArgs(orientation, rtmpAddr, streamKey)
...
func (r *TestRuntime) ffmpegExitDetail(err error) string {
	...
	r.mu.RUnlock()
	return formatFFmpegExitMessage(err, detail)
}

func formatFFmpegExitMessage(err error, detail string) string {
	errText := ""
	if err != nil {
		errText = err.Error()
	}
	...
	hint := ""
	// 退出码可能被截断为 8 位，187 本身不能唯一确定 Winsock 错误。
	// 只有原始诊断明确包含错误名或错误号时才解释它。
	if strings.Contains(detail, "-10053") || strings.Contains(detail, "WSAECONNABORTED") {
		hint = "Windows 中止了现有网络连接（WSAECONNABORTED）"
	}

	if hint != "" {
		if detail != "" && !strings.Contains(detail, hint) {
			detail = hint + "：" + detail
		} else if detail == "" {
			detail = hint
		}
	...
	if errText != "" {
		if detail != "" {
			return "FFmpeg 测试源已退出：" + errText + "，" + detail
		}
		return "FFmpeg 测试源已退出：" + errText
	}
	if detail != "" {
		return detail
	}
