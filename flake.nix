      ...
      packages = forAllSystems (
        ...
        let
          package = pkgs.buildGoModule {
            ...
            go = pkgs.go_1_26;
            vendorHash = "sha256-FAPfyjZwVCaEcsbGfKEuz0CF3HCYoG354CJ6C9grsHw=";
            nativeBuildInputs = [ pkgs.installShellFiles ];
            postInstall = ''
              installShellCompletion --cmd bili-live-tui \
                --bash <($out/bin/bili-live-tui completion bash) \
                --fish <($out/bin/bili-live-tui completion fish) \
                --zsh <($out/bin/bili-live-tui completion zsh)
            '';
